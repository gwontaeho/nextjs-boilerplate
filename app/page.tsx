...
export default function Home() {
    ...
    */
        ...
        return new Promise(async (resolve) => {
                ...
                reduced1.map((data: any) => {
                    return new Promise((resolve) => {
                        ...
                        naver.maps.Service.geocode({ query: store_address }, function (status, response) {
                            ...
                            resolve({ ...response.v2.addresses[0], ...data });
                        });
                    });
                })
            );
            ...
            for (let i = 0; i < mapped1.length; i++) {
                ...
                var marker = new naver.maps.Marker({
                    ...
                    map: mapRef.current,
                    icon: {
                        content: `<div id="paik-marker"></div>`,
                        //@ts-ignore
    ...
    const getMega2 = async () => {
        try {
            const res = await axios.get("/mega/store_search.php?store_search=%EB%8C%80%EC%A0%84");

            const megaHidden = document.getElementById("megaHidden");
            if (megaHidden) megaHidden.innerHTML = res.data;

            const mapped = Array.from(document.getElementsByClassName("cont_text")).map((_) => {
                const store_name = String(_.getElementsByTagName("b")[0].innerHTML);
                const store_address = String(_.getElementsByClassName("cont_text_info")[0].innerHTML)
                    .replaceAll("\t", "")
                    .replaceAll("\n", "");
            ...
            const mapped1 = settled.map((_: any) => {
                return _.value;
            });
            ...
            for (let i = 0; i < mapped1.length; i++) {
                ...
                var marker = new naver.maps.Marker({
                    ...
                    map: mapRef.current,
                    icon: {
                        content: `<div id="mega-marker"></div>`,
                        //@ts-ignore
                        size: new naver.maps.Size(22, 35),
                        //@ts-ignore
                        anchor: new naver.maps.Point(11, 35),
    ...
    return (
        <main className="flex w-screen h-screen">
            <div id="map" className="relative w-[2000px]"></div>
